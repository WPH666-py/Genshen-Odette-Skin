# AGENTS.md — 原神 奥黛塔（Odette）动态皮肤 · AI 安装指引

> **给 AI 的话**：用户把本仓库地址发给你，并说「安装奥黛塔皮肤 / 换个奥黛塔壁纸」时，
> **你就是安装器**。按下面步骤替用户做，不要让用户自己敲命令。

## 这套东西是什么

**原神 奥黛塔（Odette）动态皮肤** —— 三张壁纸可切换；点击皮肤播放元素爆发语音
与全套爆发特效（闪光 / 光晕 / 冲击波 / 三道斩光 / 星屑碎片 / 技能名与台词）。
交互与丝柯克皮肤一致，配色为奥黛塔主题色（粉 `#6a7bba`）。
...
| VSCode / Trae / CodeX / Cursor / Windsurf | `vscode-extension/genshen-odette-skin-1.0.0.vsix` |
| PyCharm / IntelliJ / WebStorm | `dsh-plugin/素材/wall1..3.jpg` → Background Image |
| Windows 桌面桌宠 | `desktop/奥黛塔皮肤.ps1` |
...
genshen-skin list                    # 确认 odette 在列
genshen-skin install odette         # 一键
```

### B. 单环境

```bash
genshen-skin wallpaper odette 1     # 换第 1 张壁纸（1|2|3|random）
genshen-skin ide odette             # 装 VSIX
genshen-skin pet odette             # 部署并启动桌面桌宠
genshen-skin dsh odette             # 生成 DSH 载荷 + DEFINE.md
genshen-skin deepking odette        # 生成 DeepKing 皮肤
```

### C. 手动

```
code --install-extension <仓库>/vscode-extension/genshen-odette-skin-1.0.0.vsix
```
...
2. `cordis_define`：
   - `plugin.kind = "new"`，`idPrefix = "odett"`
   - `name = "奥黛塔动态皮肤"`
   - `purpose = "原神奥黛塔动态皮肤：点击释放元素爆发语音与特效，可切换三张壁纸"`
   - `code.client` = 上面读到的源码全文
3. `cordis_run`（mode `run`）。若返回 awaiting-approval，提示用户在卡片上允许。
4. 告知用户：页面右下角出现奥黛塔挂件；左键点击释放元素爆发（语音 + 特效），
...
```
start "" powershell.exe -NoProfile -STA -ExecutionPolicy Bypass -File "<目录>\奥黛塔皮肤.ps1"
```
...
**元素爆发语音目前沿用了绫华的同款语音文件作为占位** —— 原神官方暂未提供奥黛塔的
大招语音素材。特效、交互、配色均为奥黛塔专属，只有那一声语音是沿用的。

如果用户问起或提供了奥黛塔的官方语音，替换：
...
- 三张壁纸均来自用户提供的立绘（其一为原图；其二为素绘立绘；其三为校服立绘）。
- 本皮肤 id `odette`、扩展 ID `wp666.genshen-odette-skin`，与其它皮肤互不冲突。
